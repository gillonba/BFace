using BarronGillon.BFace.YOLOv5NET.Extensions;
using BarronGillon.BFace.YOLOv5NET.Models;
...
namespace BarronGillon.BFace.YOLOv5NET {
    ...
    public class YoloScorer<T> : IDisposable where T : YoloModel
    {
        ...
        /// </summary>
        private System.Drawing.Bitmap ResizeImage(System.Drawing.Image image)
        {
            ...
            //using (var graphics = System.Drawing.Graphics.FromImage(output))
            {
                ...
                graphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.None; // no smoothing
                graphics.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.Bilinear; // bilinear interpolation
                graphics.PixelOffsetMode = System.Drawing.Drawing2D.PixelOffsetMode.Half; // half pixel offset
        ...
        /// </summary>
        private Tensor<float> ExtractPixels(System.Drawing.Image image)
        {
        ...
        /// </summary>
        private DenseTensor<float>[] Inference(System.Drawing.Image image)
        {
        ...
        /// </summary>
        private List<YoloPrediction> ParseDetect(DenseTensor<float> output, System.Drawing.Image image)
        {
            ...
            Parallel.For(0, (int)output.Length / _model.Dimensions, (i) =>
            {
                ...
                Parallel.For(5, _model.Dimensions, (k) =>
                {
                    ...
                    {
                        Rectangle = new System.Drawing.RectangleF(xMin, yMin, xMax - xMin, yMax - yMin)
                    };
        ...
        /// </summary>
        private List<YoloPrediction> ParseSigmoid(DenseTensor<float>[] output, System.Drawing.Image image)
        {
            var result = new ConcurrentBag<YoloPrediction>();
        ...
        /// </summary>
        private List<YoloPrediction> ParseOutput(DenseTensor<float>[] output, System.Drawing.Image image)
        {
        ...
        private List<YoloPrediction> Supress(List<YoloPrediction> items)
        {
            ...
            foreach (var item in items) // iterate every prediction
            {
                foreach (var current in result.ToList()) // make a copy for each iteration
                {
                    ...
                    System.Drawing.RectangleF intersection = System.Drawing.RectangleF.Intersect(rect1, rect2);
