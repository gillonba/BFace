...
namespace BarronGillon.BFace.YOLO {
    ...
    /// </summary>
    /// <remarks>
    /// Borrowed mostly intact from https://github.com/BobLd/YOLOv4MLNet/tree/yolo-v5-incl
    /// </remarks>
    public class YOLOv5_Prediction {
        /// <summary>
        /// Identity
        /// </summary>
        ...
        public IReadOnlyList<YOLOv5_Result> GetResults(string[] categories, float scoreThres = 0.5f, float iouThres = 0.5f)
        {
        ...
        private List<YOLOv5_Result> ApplyNMS(List<float[]> postProcessedResults, string[] categories, float iouThres = 0.5f)
        {
            postProcessedResults = postProcessedResults.OrderByDescending(x => x[4]).ToList(); // sort by confidence
            List<YOLOv5_Result> resultsNms = new List<YOLOv5_Result>();
            ...
            while (f < postProcessedResults.Count)
            {
                ...
                resultsNms.Add(new YOLOv5_Result(res.Take(4).ToArray(), label, conf));
